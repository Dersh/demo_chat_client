...
5.1. Подключаюсь к сокету с именем - получаю в ответ сообщение "Пришел {Имя}"
5.2. Кидаю мусор - получаю Exception
5.3. Кидаю в сокет сообщение - получаю его же в ответ
5.4. Инициирую 2 сокета: в один кидаю сообщение, в другом его считываю
...
4.1. восстанавливаем работу сокета
4.2. загружаем историю сообщений: GET https://nane.tada.team/api/rooms/{name}/history
4.3. пересылаем все сообщения в списке ожидания
4.4. При получении ответа от сервера удаляем все те сообщения, что мы отправили по id (id = GUID)
5. Проверяем запуск в оффлайн
5.1. Должна быть возможность войти в чат (кэш чата)
5.2. Должна быть возможность отправить сообщения
